import 'app_localizations.dart';

/// The translations for Chinese (`zh`).
class AppLocalizationsZh extends AppLocalizations {
  AppLocalizationsZh([String locale = 'zh']) : super(locale);

  @override
  String get enterEmail => '請填入電子郵件';

  @override
  String get enterPassword => '請填入密碼';

  @override
  String get haveAccount => '還沒有帳號?  ';

  @override
  String get signUp => '註冊';

  @override
  String get logInCaps => '登入';

  @override
  String get forgotPassword => '忘記密碼？';

  @override
  String get english => '英文';

  @override
  String get chinese => '繁體中文';

  @override
  String get indonesian => '印尼文';

  @override
  String get vietnamese => '越南文';

  @override
  String get malay => '馬來文';

  @override
  String get home => '主畫面';

  @override
  String get profile => '個人';

  @override
  String get explore => '探索';

  @override
  String get createItinerary => '建立行程';

  @override
  String get notification => '通知';

  @override
  String get settings => '設定';

  @override
  String get language => '語言';

  @override
  String get lightMode => '淺色模式';

  @override
  String get darkMode => '深色模式';

  @override
  String get logOut => '登出';

  @override
  String get aboutUs => '關於我們';

  @override
  String get posts => '帖文';

  @override
  String get ratings => '回饋';

  @override
  String get commets => '評論';

  @override
  String get followers => '追踪者';

  @override
  String get following => '追踪';
  @override
  String get AddtoFavorite => '加入收藏夾';
  @override
  String get Photos => '相片';
  @override
  String get MoreInformation => '更多資訊';
  @override
  String get Comments => '評論';
  @override
  String get AddtoComment => '新增評論...';
  @override
  String get AddComment => '新增評論';
  @override
  String get greatPlace => '好地方！';
  @override
  String get loveTheAtmosphere => '喜歡這裡的氣氛。';
  @override
  String get recommendVisiting => '建議參觀。';
  @override
  String get vietnam => "越南";
  @override
  String get currentTemperature =>"  目前溫度";
  @override
  String get moreDetail => '更多詳情';
  @override
  String get exchangeRate => '  匯率';
  @override
  String get destinations => '  目的地';
  @override
  String get restaurants => '  餐廳';
  @override
  String get hotels => '  飯店';
  @override
  String get img =>'assets/images/events/Da_nang1.jfif';
  @override
  String get like => '讚';
  //indonesia
  @override
  String get BorobudurTemple => '菩提浮屠';
  @override
  String get borobudurTempleDescription => 'Candi Borobudur 或婆羅浮屠寺是印尼聯合國教科文組織世界遺產，以其令人驚嘆的世界上最大的佛教寺廟建築群而聞名。 它以複雜的石雕為特色，讓您可以一睹古代爪哇的歷史和文化。';

  String get indonesia => '印尼';
  String get baliIsland => '峇里島';
  String get jakartaOldTown => '雅加達老城';
  String get fourSeasons => '峇里四季酒店';
  String get gaiaHotel => 'Gaia酒店萬隆';
  String get kempinski => '印尼雅加達凱賓斯基酒店';
  String get bandarDjakarta => 'Bandar Djakarta';
  String get blueTerrace => 'Blue Terrace';
  String get kudeta => 'Ku De Ta';

  //taiwan
  String get taiwan => '台灣';

  //vietnam
  @override
  String get DanangScreen => '峴港';
  @override
  String get DanangDescription => '峴港坐落在越南中部海岸，是一座對比鮮明的城市，將歷史與現代完美地融合在一起。 從古老的佔婆帝國根源到充滿活力的城市景觀，峴港為遊客提供了多樣化的體驗。 這座城市的歷史在美溪海灘的佔族文明遺跡和五行山周圍的戰時遺跡中得以體現。 然而，峴港以引人注目的現代建築迎接 21 世紀，龍橋等地標建築就是例證。';
  @override
  String get DanangOpenHours => '開放時間：上午 9:00 - 下午 6:00';
  @override
  String get DanangAddress => '地址：越南峴港';
  @override
  String get DanangContact => '聯絡方式：+84 456 789';
  @override
  String get haLongBay => '下龍灣';
  @override
  String get haLongBayDescription => '下龍灣是聯合國教科文組織世界遺產，坐落在北部灣，是一處令人著迷的自然奇觀，以其碧綠的海水、數千個石灰岩喀斯特地形和漂浮的漁村吸引著遊客。下龍灣以其超凡脫俗的美麗和文化意義屹立不倒。作為大自然奇蹟的證明。 無論是遊覽喀斯特地形、探索隱藏的洞穴，還是與當地社區互動，下龍灣的遊客都會在越南最具標誌性的目的地之一獲得難忘的體驗。 在旅行之前，請考慮查看當地旅遊資訊以獲取最新的更新和指南。';
  @override
  String get haLongBayAddress => '地址：越南廣寧省';
  @override
  String get haiPhong => '海防';
  @override
  String get haiphongDescription => '越南海防是一座沿海城市，以其充滿活力的活力和歷史魅力而聞名，獨特地融合了豐富的文化和美麗的風景。 海防以其充滿活力的氛圍和多樣化的景點歡迎遊客探索其文化遺產和自然奇觀。 在您參觀之前，建議您查看景點的具體開放時間，如果需要任何幫助或信息，可以隨時向海防旅遊局和當地旅遊信息中心尋求幫助。';
  @override
  String get haiPhongAddress => '地址： 越南海防';
  @override
  String get edenHotels => '伊甸園酒店';
  @override
  String get edenDescription => '伊甸園酒店 (Eden Hotels) 坐落在越南充滿活力的風景之中，體現了奢華與寧靜，提供舒適而精緻的天堂。 伊甸園酒店 (Eden Hotels) 坐擁全國繁華城市之一的黃金地段，也可能位於寧靜的沿海度假勝地，以精緻的待客之道歡迎賓客來到這裡。 建築將現代優雅與當地文化影響完美融合，營造出獨特的沉浸式氛圍。 設計無可挑剔的客房和套房完美融合了舒適與時尚，享有周圍美景的全景。 伊甸園酒店的美食體驗是一場美食之旅，在精緻的餐飲場所供應各種國際和當地美食。 客人可以在最先進的水療和健身設施中放鬆身心並恢復活力，或在鬱鬱蔥蔥的綠色植物環繞的室外游泳池中盡情享受。 無論是商務還是休閒，越南伊甸園酒店都為豪華住宿樹立了新標準，以無可挑剔的服務和對細節的關注為您帶來難忘的住宿體驗。';
  @override
  String get metooHomestay => "米圖民宿";
  @override
  String get metooDescription => 'Metoo Homestay 坐落在越南的中心地帶，體現了越南傳統款待的精髓。 這家迷人的寄宿家庭為客人提供了獨特的機會，讓他們沉浸在當地文化中，同時享受賓至如歸的舒適。 Metoo Homestay 的質樸建築和溫馨的氛圍營造出溫馨的氛圍，讓遊客感覺就像是社區的一部分。 寄宿家庭旨在提供真正的文化體驗，讓客人與友好的當地人互動並品嚐正宗的越南美食。 設備齊全的客房體現了現代便利與傳統美學的融合，確保您享受舒適而難忘的住宿體驗。 無論是在寧靜的庭院裡品嚐現煮的越南咖啡，還是參加動手烹飪課程以掌握製作當地菜餚的藝術，Metoo Homestay 超越了典型的住宿，提供豐富而身臨其境的旅行體驗。';
  @override
  String get MetooAddress => '地址：越南大叻';
  @override
  String get raonHotels => '拉昂飯店';
  @override
  String get raonDescription => '越南拉昂酒店 (RAON Hotels) 因其致力於提供奢華而難忘的酒店體驗而聞名。 這些飯店坐落在全國各地的重要地點，將現代優雅與越南魅力完美融合。 從客人踏入時尚大廳的那一刻起，他們就會感受到溫暖溫馨的氛圍。 住宿體現了當代設計與傳統美學的和諧融合，確保舒適性和文化真實性。 RAON Hotels 的標誌之一是致力於提供無可挑剔的服務。 專業且細心的工作人員盡力滿足每位客人的需求，營造出處處洋溢著熱情好客的氛圍。 飯店擁有一系列設施，包括設備齊全的客房、最先進的設施以及展示越南美食豐富風味的精緻餐飲選擇。';
  @override
  String get eraRes => '時代餐廳';
  @override
  String get eraDescription => '越南的 Era 餐廳濃縮了該國豐富的烹飪傳統的精髓，同時將其與現代餐飲體驗無縫融合。 Era 餐廳坐落在越南的中心地帶，是一個美食天堂，讓食客穿越時空，提供跨越幾代人的美食之旅。 餐廳的氛圍將越南傳統建築元素與現代設計和諧融合，營造出溫馨而優雅的氛圍。 當顧客進入 Era 時，他們會受到開放式廚房散發出的芳香交響樂的歡迎，熟練的廚師精心烹製的菜餚展示了越南多樣化的地方風味。 Era 餐廳的菜單堪稱烹飪藝術的瑰寶，精選菜餚彰顯了該國充滿活力的街頭美食文化，以及經典越南食譜的精緻演繹。 從充滿香草的芬芳河粉到充滿新鮮食材的精緻春捲，每道菜都證明了餐廳對正宗和創新的承諾。 Era 為採購優質的本地原料、為全國各地的農民和生產者提供支援而感到自豪。';
  @override
  String get terracoSkyBar => 'Terraco 空中酒吧和餐廳';
  @override
  String get terracoSkyBarDescription => 'Terraco Sky 酒吧及餐廳坐落在越南的中心地帶，是美食和社交樂趣的燈塔。 這家餐廳坐落在充滿活力的城市景觀之上，提供無與倫比的餐飲體驗，並可欣賞下方繁華街道的全景。 氛圍將精緻與溫馨完美融合，採用現代裝飾，與令人驚嘆的城市燈光背景相得益彰。 當您踏上露臺時，活潑音樂的節奏為您度過一個難忘的夜晚。 酒吧提供各種手工調製的雞尾酒，每一款本身就是傑作。 從經典調酒到調酒師的創意特寫，每一口都是風味之旅。 餐廳區提供的菜單既體現了越南烹飪傳統，又融入了現代風味，可滿足挑剔的味蕾。 無論您是享受浪漫晚餐、與朋友聚會或慶祝特殊場合，Terraco Sky 酒吧及餐廳都能為您提供溫馨的空間。 細心的工作人員確保每位客人不僅感受到被照顧，而且感受到真正的關懷。 憑藉其獨特的氛圍、卓越的烹飪和令人驚嘆的景觀的融合，該場所將為那些在越南尋求難忘夜晚的人們帶來更高的體驗。';
  @override
  String get caugoRes => '考高餐廳';
  @override
  String get caugoResDescription => 'Cau Go 餐廳坐落在越南的中心地帶，以其迷人的氛圍和美味的菜餚體現了越南美食的精髓。 這間美食瑰寶位於充滿活力的河內市，坐落在還劍湖邊緣，可欣賞到令人驚嘆的全景，成為令人難忘的用餐體驗的迷人背景。 餐廳採用越南傳統裝飾，散發著溫暖和真實的氣息，為尋求融入越南豐富文化的顧客營造出溫馨的氛圍。 當您踏入 Cau Go 時，您就會感受到越南香料的誘人香氣和現場傳統音樂的舒緩旋律。 餐廳內部以深色木製家具和柔和的燈光為特色，將客人帶入時間放慢的境界，讓客人悠閒地探索越南的美食寶藏。 菜單是穿越該國不同地區的美食之旅，包括河粉、越南三明治和新鮮海鮮等招牌菜餚。 每道菜都是傑作，精心呈現，體現了越南美食對細節的一絲不苟。';

  //malaysia
  String get malaysia => '馬來西亞';

  @override
  String get LangkawiScreen => '浮罗交怡岛';
  String get LangkawiDescription => '一個美麗的群島，蘭卡威島是以其廣闊的沙灘和湛藍的海水而聞名，吸引遊客。蘭卡威地質公園是一個吸引大量遊客的聯合國教科文組織遺址，無疑是馬來西亞必遊景點中的首選之一。它完美地展示了該地區的地質遺產。它是家人一起遊覽馬來西亞的最佳地點之一。';
  String get LangkawiAddress => '地址：浮罗交怡岛,馬來西亞';
  @override
  String get MountKinabaluScreen => '金寶山';
  @override
  String get MountKinabaluDescription => '婆罗洲最高的山脉，金寶山是一道引人注目的景观。登山者喜欢走上壮观的山径，一睹该地区如画的景色。广受爱好冒险的人群喜爱，是马来西亚最佳的探险胜地之一。在马来西亚旅游景点中名列前茅。';
  @override
  String get MountKinabaluAddress => '地址：金寶山，馬來西亞';
  @override
  String get MantananiScreen => '曼坦尼';

  @override
  String get MantananiDescription => '位於Kota Belud附近的沙巴曼坦尼群島是馬來西亞最著名的旅遊勝地之一，距離哥打京那巴魯僅一小時車程，提供婆羅洲最優質的潛水場所。這是馬來西亞最著名的地方之一，因其潛水景點而聞名，也是所有浮潛和潛水愛好者的最愛，他們從首都來此島進行一日遊。潛水者大量涌入這些島嶼，來自世界各地，在清澈的水中漫遊，與海馬和魟魚相遇。在這裡，有超過20個地點供應潛水運動，適合擁有潛水證書的潛水愛好者。';

  @override
  String get MantananiAddress => '地址：曼坦尼，馬來西亞';
  @override
  String get MovHotels => 'Mov 酒店';

  @override
  String get MovHotelsDescription => '坐落在武吉免登的中心地带，该酒店方便前往各种餐馆、购物中心和令人兴奋的美食街。拥有现代化和美观的客房、景观优美的屋顶游泳池以及友好的员工，是舒适便利的理想选择。';
  @override
  String get SunwayPutraHotel => '双威普特拉酒店';

  @override
  String get SunwayPutraHotelDescription => '迷人的酒店与购物商场相连，提供便利的餐饮和公共交通。拥有宽敞、干净的客房，可提供免费升级，令人印象深刻的景观，以及舒适的现场咖啡屋。优越的位置靠近PWTC车站，便于探索城市。';
  @override
  String get PacificSuteraHotel => '太平洋素特拉酒店';

  @override
  String get PacificSuteraHotelDescription => '豪華度假酒店，享有美麗的海景和花園景觀。乾淨、高雅的客房，帶有本地風味。設有私人海灘、高爾夫球場、游泳池和兒童遊樂場。多家餐廳供選擇，包括Al Fresco、Silk Garden和Cafe Boleh。令人難忘的住宿體驗，樂於助人的工作人員。';
  @override
  String get EnakKL => 'Enak KL';

  @override
  String get EnakKLDescription => 'Enak位於吉隆坡Starhill Gallery內，是一家高級的馬來餐廳。作為吉隆坡最好的用餐地點之一，您可以品嚐到最美味的馬來菜餚，其中許多菜餚都是基於Sherena Razaly家族代代相傳的食譜而製作的，她是餐廳的老板。儘管Enak的招牌菜色如Ayam Goreng Lengkuas（藍薑炸雞）等口味都非常傳統，但Enak無疑是吉隆坡最好的馬來西亞餐廳，在您造訪城市時絕對是必去之地。';
  @override
  String get RestoranSamy => 'Restoran Samy';

  @override
  String get RestoranSamyDescription => '在馬來西亞尋找頂級的印度餐廳？如果您想嚐嚐正宗的印度菜，不妨造訪怡保的Restoran Samy。這家餐廳相當於吉隆坡的Devi’s Corner，在經營超過20年後，Restoran Samy提供一流的香蕉葉飯，以及令人驚艷的素食和非素食菜餚。';
  @override
  String get LeQueRestaurant => 'Le Que 餐廳';

  @override
  String get LeQueRestaurantDescription => 'Le Que 餐廳成立於2014年，相對於全國許多其他餐廳，可能歷史較短，但您總是可以期待由訓練有素且經驗豐富的廚師烹製的美食。這是一家融合咖啡館，您可以品嚐到各種西式料理和豐富的義大利麵選擇。如果您在馬六甲市並且想吃義大利麵，絕對應該前往 Le Que。';



//taiwan
  @override
  String get kenting => '墾丁';
  @override
  String get kentingDescription => '墾丁位於台灣南端，是個熱帶天堂，輕鬆地將自然美景與充滿活力的文化融為一體。 墾丁以其原始的海灘、鬱鬱蔥蔥的景觀和無數的戶外活動而聞名，已成為本地和國際遊客追捧的目的地。 該地區擁有多樣化的生態系統，從充滿海洋生物的珊瑚礁到為獨特動植物提供棲息地的茂密森林。 墾丁的主要吸引力之一是其令人驚嘆的海岸線，金色的沙灘與太平洋的藍色海水交匯在一起。 白沙和南灣等白色沙灘邀請遊客在陽光下放鬆身心，或參與從衝浪到浮潛等刺激的水上運動。 海岸線之外，墾丁國家公園是自然愛好者的天堂，這裡的健行步道蜿蜒穿過亞熱帶森林，通往全景觀景台，展示該地區令人驚嘆的風景。';
  @override
  String get kinmen => '金門';
  @override
  String get kinmenDescription =>'金門，正式名稱為金門縣，是位於中國東南沿海的一群島嶼，由台灣管轄。 金門以其豐富的歷史和戰略意義而聞名，在台灣與大陸的兩岸動態中發揮著舉足輕重的作用。 該群島由多個小島組成，其特點是傳統福建文化與現代發展的獨特融合。 來到金門的遊客會被迷人的古代建築組合所吸引，例如歷史悠久的軍事防禦工事和傳統閩南風格的房屋，與現代基礎設施的背景並置金門的歷史重要性深植於它在冷戰時期的作用，它是抵禦中國大陸潛在入侵的前線防禦。 如今，這段軍事歷史的遺跡散佈在島嶼各處，包括標誌性的風獅神，這些石像被認為可以保護居民免受邪靈和強風的侵害。 金門國家公園保留了許多這樣的歷史遺跡，讓遊客在欣賞風景如畫的同時，深入了解該地區的過去。 除了歷史意義外，金門還以其充滿活力的文化遺產而聞名。 島民保留了自己獨特的方言、習俗和儀式，創造了金門有別於台灣其他地區的文化掛毯。 媽祖朝聖和城隍祭等傳統節日可以讓您一窺島上的精神和公共生活。 此外，金門的美食反映了其沿海位置，以一系列新鮮的海鮮菜餚為特色，展示了島上的烹飪實力。';
  @override
  String get wulai => '烏來';
  @override
  String get wulaiDescription => '烏來坐落在台灣北部鬱鬱蔥蔥的山脈中，是一個風景如畫的土著村莊，以其自然美景、豐富的文化和舒緩的溫泉而聞名。 作為泰雅族祖先土地的一部分，烏來提供了了解台灣原住民遺產的獨特機會。 這個村莊被茂密的森林和波光粼粼的南勢河所包圍，為尋求遠離城市生活的遊客創造了寧靜的背景。 烏來的主要景點之一是其令人煥發活力的溫泉，吸引著尋求放鬆的當地人和遊客。 地熱水被認為具有治療功效，使泡在露天浴池中成為一種治療和身臨其境的體驗。 遊客還可以探索風景優美的烏來瀑布，瀑布從 80 公尺高處飛瀉而下，沿著一條蜿蜒穿過翠綠森林的風景小徑進入。 烏來不僅是大自然愛好者的天堂，也是大自然愛好者的天堂。 它還提供了對土著生活方式的一瞥。 泰雅族原住民博物館提供對部落傳統的全面了解，展示文物、傳統服裝和互動展品，講述泰雅族豐富的文化歷史。';
  @override
  String get grandTaipei => '台北君悅酒店';
  @override
  String get grandTaipeiDescription =>'台北君悅酒店是一家豪華且標誌性的五星級飯店，位於台灣充滿活力的首都城市中心。 飯店位於信義區的黃金地段，為商務和休閒旅客提供無與倫比的便利。 台北君悅酒店擁有引人注目的現代建築設計，矗立在城市天際線上，成為精緻與時尚的象徵。 進入酒店後，客人首先會看到令人印象深刻的大廳，大廳裝飾有當代藝術品和優雅的家具。 室內設計對細節的關注體現了酒店致力於為客人提供奢華舒適的環境的承諾。 寬敞且設備齊全的客房和套房可欣賞台北市的壯麗景色，為那些尋求遠離喧囂城市景觀的客人提供寧靜的休憩之所。 台北君悅酒店不僅以其豪華的住宿環境而聞名，還以其世界一流的餐飲選擇而聞名。 飯店擁有多家餐廳，可滿足不同的烹飪喜好，從正宗的當地美食到由著名廚師烹製的國際美食。 賓客足不出戶即可享受美食之旅，在精緻溫馨的氛圍中品嚐最美味的菜餚。 除了豪華的住宿和餐飲體驗外，台北君悅酒店還配備最先進的設施，適合商務會議、會議和社交活動。 飯店的活動空間用途廣泛，可為不同規模的聚會提供彈性。 無論舉辦公司活動或盛大慶典，飯店敬業的員工都會確保每個細節都經過精心管理，超越挑剔客人的期望。';
  @override
  String get workinn => '台北101的WORK INN';
  @override
  String get workinnDescription => '嵌入台北繁華的心臟地帶，台北101的WORK INN是一個充滿活力和動感的共享工作空間，巧妙地融合現代功能和城市豐富文化的經典。位於標誌性的台北101摩天大樓內，這個創新的工作空間為專業人士和自由職業者提供了一個獨特且鼓舞人心的環境，培養創造力和提高生產力。WORK INN引以為傲的現代設施、人體工學工作站以及城市景觀的全景視野，為合作努力提供了理想的場所。內部設計體現了當代美學和傳統台灣元素的和諧融合，營造出一種引人入勝且激發靈感的氛圍。當你沉浸在這個共享工作的天堂的日常節奏中時，你將發現一個多元化的社群，由志同道合的個體組成，促進了網絡機會和共享抱負的情感。無論你是當地企業家還是探索台北的數字遊牧民，台北101的WORK INN概括了現代工作空間的本質，包含創新、連通性和台北蓬勃的商業文化精神。';
  @override
  String get hanns => '瀚寓酒店」';
  @override
  String get hannsDescription =>'Tọa lạc tại Đài Bắc, Hanns House là nơi khám phá văn hóa đáp ứng nhu cầu tham quan của du khách. Trên toàn cầu, Hanns House đại diện cho người tiên phong trong việc tận dụng sự sáng tạo để đáp ứng nhu cầu đa dạng của khách doanh nhân. Vượt qua các ngành công nghiệp tài chính và công nghệ, thăng tiến từ lĩnh vực sáng tạo sang kinh doanh xuất bản và cuối cùng tiến tới thế giới quản lý khách sạn, bà Teresa Wei-Hsin Ma còn được biết đến là một nữ doanh nhân “Slashie” ở thành phố Đài Bắc. Thiết kế nội thất của Hanns House được lấy cảm hứng từ ông nội của bà Ma. Với hy vọng kết nối hiện thực và lý tưởng, Hanns House mang đến cho những vị khách quý của mình một không gian thoải mái và thích thú, mang lại trải nghiệm cảm giác truyền thống và hiện đại cho du khách.';
  @override
  String get hannsContact =>'聯絡: +886-2-8758-3777';
  @override
  String get hannsAddress =>'地址: 台北市信義區基隆路一段206號';
  @override
  String get mosun => '摩順鐵板燒';
  @override
  String get mosunDescription => '莫山鐵板燒坐落在台灣充滿活力的美食氛圍中，是一個美食天堂，巧妙地將日本料理藝術與台灣風味融為一體。 踏入餐廳，迎面而來的是鐵板肉的誘人香氣和鍋鏟在熱鍋上舞動的節奏聲。 鐵板燒廚師是技藝大師，他們以令人著迷的精確度和表演技巧展示他們的烹飪高超。 Mosun Teppanyaki 的氛圍是現代優雅與傳統日本美學的和諧結合。 鐵板燒餐桌經過精心佈置，旨在提供私密的用餐體驗，讓顧客可以近距離目睹烹飪表演。 菜單包含一系列優質食材，從多汁的和牛到當地採購的新鮮海鮮，保證為您帶來一場感官盛宴。 在Mosun鐵板燒，用餐體驗超越了單純的用餐； 它變成了一場在您眼前精心製作的風味和質地之旅。 技藝高超的廚師不僅烹飪精準，還為食客帶來互動式的美食盛宴，讓每次光臨都成為難忘的時刻。 無論是獨自享受美食之旅，還是與朋友和家人分享體驗，莫山鐵板燒誠邀請賓客品嚐日本烹飪傳統與台灣待客之道的完美結合。';
  @override
  String get shabu => '食令Shabu';
  @override
  String get shabuDescription => '涮鍋是一種流行的日本火鍋菜餚，在台灣廣受歡迎。 通常，它包括切成薄片的肉，通常是牛肉或豬肉，以及各種新鮮蔬菜、豆腐和麵條。 「Shabu-Shabu」一詞指的是原料在沸騰的肉湯中短暫煮熟時發出的嗖嗖聲。 在台灣，涮鍋通常在公共環境中享用，食客們聚集在桌子中央的燉鍋周圍。 湯底可以有所不同，提供味噌、大豆或昆布（海藻）底等選擇。 此外，還提供各種蘸醬以增加風味。 涮鍋的互動性和社交性使其成為朋友和家人一起用餐的熱門選擇。 對新鮮、高品質食材的重視以及菜餚的可自訂性使其在台灣充滿活力的烹飪界中具有吸引力。';
  @override
  String get matsu => '樂軒松阪亭  ';
  @override
  String get matsuDescription => '松阪亭坐落在充滿活力的台北信義區，是吸引當地人和遊客的美食瑰寶。 這家餐飲天堂位於著名的新光三越信義大樓內的新天地六樓，營造出精緻而溫馨的氛圍。 當您登上餐廳時，您會看到現代設計與傳統風格融合的氛圍，營造出既現代又富有文化氣息的氛圍。 松坂亭以其精緻的美食而聞名，顧名思義，它可能專注於日本料理。 菜單經過精心策劃，以展示最好的口味，也許以頂級松阪牛肉作為主菜。 細心的工作人員和繁華的信義區的全景盡收眼底，顧客不僅可以享受美食，還可以享受身臨其境的用餐體驗。';
  @override
  String get matsuContact => '聯絡方式: +886 2 2345 8833';
  @override
  String get matsuAddress => '地址: 松高路19號新光三越信義新天地A4館6F, Taipei, Taiwan';
  @override
  String get deletephoto => '刪除照片';
  @override
  String get checkphoto => '查看照片';
  @override
  String get Noimageavailable => '無圖片可用';
  @override
  String get PersonalityTest => '個性測試';
  @override
  String get EditProfile => '編輯個人資料';
  @override
  String get Youdonthaveanyratingsyet => '你還沒有任何評分';

  @override
  String get Youhaventleaveanycommentsyet => '你還沒有留下任何評論';
  @override
  String get N1 => '自然景觀';

  @override
  String get N2 => '購物商場';

  @override
  String get N3 => '歷史古蹟';

  @override
  String get N4 => '宗教場所';

  @override
  String get N5 => '主題樂園';
  @override
  String get A1 => 'Sumatra';

  @override
  String get A2 => 'Java';

  @override
  String get A3 => 'Kalimantan';

  @override
  String get A4 => 'Sulawesi';

  @override
  String get A5 => 'Pupua';

  @override
  String get A6 => '北部';

  @override
  String get A7 => '中部';

  @override
  String get A8 => '南部';

  @override
  String get A9 => '東部';
  @override
  String get itenary => '行程';
  @override
  String get Selectacountry => '選擇國家';
  @override
  String get Jakarta => '雅加達';

  @override
  String get JakartaScreen => '雅加達';

  @override
  String get JakartaDescription =>
      '以印度尼西亞最繁忙的大都市聞名，雅加達是一座擁有來自印尼各地多樣族群背景的一千萬人口的城市，他們和睦共處。漫步在城市街道上，欣賞都市的景色。在豪華購物商場尋找優惠。沉浸在小巷中，發現街頭小攤的美食寶藏。';
  @override
  String get LabuanBajo => '峇厘巴露';

  @override
  String get LabuanBajoDescription =>
      '峇厘巴露其實只是位於佛羅勒斯島西端的一個小漁村。現在，隨著科莫多國家公園的知名度，這個地方已成為東努沙登加拉省無數目的旅遊地之一。屬於科莫多國家公園中眾多目的地之一的峇厘巴露，總是您旅程的起點，因為這裡是港口和機場所在地。';
  @override
  String get PadmaResort => 'Padma度假村';

  @override
  String get PadmaResortDescription =>
      '坐落在一片宁静的竹林中，这家豪华酒店拥有一座引人注目的无边泳池和可供游泳的酒吧，享有巴厘岛最佳美景和迷人景色。享受宽敞的客房、出色的餐饮、丛林徒步旅行和竹林漫步。尽情享受现场活动，如空中瑜伽、射箭和儿童俱乐部，以迷人的小动物园氛围进行有趣的活动。';
  @override
  String get AdiwanaUnagiSuites => 'Adiwana Unagi 度假村';
  @override
  String get AdiwanaUnagiSuitesDescription => '美麗的酒店提供令人驚嘆的叢林景觀，卓越的設施和無可挑剔的服務。設有令人驚嘆的室外浴室、無邊際泳池、直接通往泳池的客房、全日供應早餐以及帶寬敞陽台的豪華森林景觀客房。強烈推薦入住，享受非凡的住宿體驗。';
  @override
  String get Kokiku => 'Feast by Kokiku';
  @override
  String get KokikuDescription => '這是另一家位於雅加達南部Senopati的最時尚且溫馨的餐廳，擁有藝術裝飾植物主題的場所。它提供各種印尼美食供分享。您一定要嘗試它的bebek betutu，用巴厘島香料烹製的鴨肉，搭配香脆的炸洋蔥或sambal matah辣椒和芬芳的米飯。';
  @override
  String get Soulfood => 'Soulfood';
  @override
  String get SoulfoodDescription => '如其名，Soulfood 提供各種在印尼家庭日常供應的美食，因此成為他們的一種靈魂。其最受歡迎的菜單包括 cumi item（墨魚墨汁烹飪）、baso darokdok（肉丸湯配水牛皮脆片）、或 teri pete lado ijo（江魚干與青辣椒和青豆炒製）。';
  @override
  String get Cameron => 'Cameron Highlands';
  @override
  String get CameronDescription => '這座山區站是馬來西亞最古老的旅遊勝地之一，建立於1930年代，現在居住著一群非常多元化的人，也對當地文化產生了影響。迷人的茶園、宜人的氣候、瀑布飛瀉、草莓農場和美麗的果園足以成為你造訪此地的理由。';
  @override
  String get Birdpark => '吉隆坡鳥園';
  @override
  String get BirdparkDescription => '位於馬來西亞吉隆坡的鳥園被劃分為四個區域，其中1區和2區供鳥類自由飛行，它們在這裡被放養飛行並提供良好的環境進行自然繁殖。在這些區域觀賞所有色彩繽紛的鳥兒一同飛翔是一種享受。吉隆坡鳥園的3區是犀鳥公園，4區包括許多鳥類的籠子和不同區域。';
  @override
  String get HardRock => '檳城硬石酒店';
  @override
  String get HardRockDescription => '優秀的服務，絕佳的位置，位於海灘旁。房間設施很好，特別是可以觀賞最新電影的選擇。房間氛圍溫馨舒適。游泳池讓每個人都很開心。';
  @override
  String get Trove => 'TROVE Johor Bahru';
  @override
  String get TroveDescription => '酒店提供乾淨寬敞的房間，配有蓬鬆的枕頭，地點靠近餐飲選擇、商場和景點。提供家庭套房。極佳的服務和房內飲水機。';
  @override
  String get Azmie => 'Azmie Wawa';
  @override
  String get AzmieDescription => '對於那些渴望品嚐馬來西亞知名甜點冰磚的人來說，一定要來到著名的Azmie Wawa。位於Kampung Melayu Majidee的美食廣場內，這是馬來西亞甜點愛好者的最佳去處。在Azmie Wawa，你可以選擇兩種類型的冰磚 - ABC Biasa和ABC Special。豐富多彩的食材是這些甜點如此特別且在當地人中非常受歡迎的原因。你會找到紅豆、仙草、attap chee、碎花生、巧克力糖漿、煉奶、各種風味的糖漿等等。';
  @override
  String get CafeBLD => 'Café BLD';
  @override
  String get CafeBLDDescription => '位於文莱酒店的 Café BLD 在柔佛提供道地的食物、五星級的氛圍和服務，全都是一種享受。 Café BLD有多個開放式廚房，烹製各種美食，如馬來、中國、印度、日本、西式、海鮮等。這裡的自助餐將滿足您的所有口腹之慾，使 Café BLD成為馬來西亞最著名的餐廳之一。';
  @override
  String get Rainbow => '彩虹村';
  @override
  String get RainbowDescription => '作為台灣最具美學意義的景點之一，彩虹村位於南通的一個小型聚落，以其古怪多彩的房屋而聞名。該村最初是一個軍屬聚居地，後來通過街頭藝術轉變而來。雖然這裡沒有顯著的景點，但穿過彩虹村色彩斑斕的街道進行步行遊覽仍然是一個必試的活動。這個努力背後的主要藝術家，被所有人親切地稱為爺爺，仍然住在村裡並隨時歡迎遊客前來拜訪。';
  @override
  String get Taroko => '太魯閣大峽谷';
  @override
  String get TarokoDescription => '沿著日本東海岸延伸，太魯閣大峽谷是一個壯觀的長達19米的峽谷。這裡擁有豐富多樣的植物和動物，現已被識別為國家公園。公園內生活著30多種動物，包括鹿、野豬和熊。這裡也可以看到各種不同的蝴蝶。山脈上有幾條遠足小徑，使其成為一個受歡迎的遠足目的地。';
  @override
  String get Dandy => 'Dandy 酒店';
  @override
  String get DandyDescription => '台北的現代精品酒店，提供乾淨的客房、舒適的床鋪和溫馨的氛圍。靠近景點、市場和交通便利。多樣化的早餐自助餐、24/7 洗衣設施和免費設施。';
  @override
  String get Caesar => '台北凱撒大飯店';
  @override
  String get CaesarDescription => '分配的 901 號房面對隔壁建築，所以景觀不怎麼吸引人。房間對於兩人和兩個行李包來說大小適中。清潔度有待改善，因為發現了灰塵，浴室地板清潔得不夠好。';
  @override
  String get Lin => '林東芳';
  @override
  String get LinDescription => '儘管外表樸素，但台北的居民會告訴您，林東芳是牛肉麵的最佳去處。半筋半腱牛肉麵湯很有名。它有著充足的高品質肉片，清淡微辣的湯頭和彈牙的麵條。';
  @override
  String get Shin => '欣葉';
  @override
  String get ShinDescription => '一家家庭餐廳從1977年在一條後巷僅有11張桌子開始，如今在台北周圍已經有5家欣葉餐廳。每家餐廳以現代化的設置供應傳統福建風味的台灣美食。它的成功和良好聲譽源於對每一個細節的關注，經驗豐富的廚師們非常重視他們所供應菜品的質量。';
  @override
  String get Hue => '順化';
  @override
  String get HueDescription => '順化是越南最具歷史意義的城鎮之一，充滿了19世紀阮朝統治時期的遺跡。坐落在美麗的香水河畔，皇城是一個巨大的遺址，被高達2.5公里長的城牆圍繞。遊覽這個地方時，可以欣賞到美麗的午門、內部精細漆飾的太和殿、越南皇太后居住的殿堂以及保存完好的文官廳壁畫。';
  @override
  String get Phong => '逢納-克邦國家公園';
  @override
  String get PhongDescription => '越南最適合洞穴探險的地方之一，被列為世界遺產的逢納-克邦國家公園是一個壯觀的岩溶山地形成，有著巨大的洞穴，內有極好的鐘乳石和石筍展示。以其洞穴活動而聞名，從多日徒步和洞穴遊覽到簡單的半日遊，可以通過船遊和現代木棧道輕鬆抵達洞穴，但國家公園也提供山地自行車和徒步旅行活動。';
  @override
  String get Jade => '東方翡翠酒店';
  @override
  String get JadeDescription => '友善的工作人員和非常好的客房。可愛的頂樓酒吧設有泳池，可欣賞城市景觀。非常包容且輕鬆的氛圍。交通非常便利，位置優越，適合城市和交通。';
  @override
  String get Hav => 'Havana Nha Trang酒店';
  @override
  String get HavDescription => '我喜歡造訪芽莊，每次都喜歡入住 Havana 芽莊。海濱酒店的價格合理。酒店乾淨，工作人員非常友好，海景泳池非常漂亮，桑拿服務營業到晚上9點。';
  @override
  String get Cuc => 'Cục Gạch Quán';
  @override
  String get CucDescription => '坐落在一幢迷人的房子裡，Cuc Gach Quan採用了帶有復古家具的鄉村設計，讓人回想起幾十年前的越南生活。菜單上充滿了反映越南家庭傳統餐飲的菜餚。如果你想品嚐正宗的越南家常菜，這是其中一個最佳的地方。';
  @override
  String get ManMoi => '滿梅';
  @override
  String get ManMoiDescription => '對於越南地區的美食深具欣賞的滿梅從傳統餐飲和地方菜餚中汲取靈感來建構菜單。提供從令人愉悅的前菜到豐富的主菜，接著是舒緩口感的湯品，以及美味的甜點來結束一頓甜美的餐點。';

  String get amount =>'金額';
  String get amountDes =>'請輸入金額';
  String get exchangerate => '匯率';
  String get from =>"從： ";
  String get to => '到： ';
  String get tranfer => '轉換';
  String get usd => '美元 (USD)';
  String get vnd => '越南盾 (VND)';
  String get twd => '新台币 (TWD)';
  String get idr => '印尼盾 (IDR)';
  String get myr => '马来西亚林吉特 (MYR)';
  String get bali => "峇里島";
  String get baliDescription => '峇裡島是印尼最著名的島嶼，以其美麗的海灘、充滿活力的夜生活和豐富的峇裡文化而聞名。 對於尋求休閒和冒險相結合的旅行者來說，這是一個頂級目的地。 ';
  String get kotaTua => "雅加達老城";
  String get kotaTuaDescription => '雅加達舊城區（印尼語：Kota Tua）是印尼雅加達地區巴達維亞（Batavia）的遺址，是荷蘭在雅加達地區建立的第一個有城牆的定居點。 它是一座有自己城堡的內城。 該地區在17世紀至19世紀間變得重要，當時它被建立為荷蘭東印度公司的事實上首都。 ';
  String get open => '開放時間：24小時';
  String get ads =>'網址：印尼峇裡島';
  String get kotaads => '地址：Taman Fatahillah No.1，Pinangsia，Kec。 Taman Sari, 西雅加達市, 雅加達首都特區 11110, 印尼';
  String get four => '四季峇里';
  String get fourDescription => '融入巴厘島神奇的能量和柔和的大自然。坐落在一個三英里長的寧靜海灣中，我們的別墅反映了巴厘島地道的村莊。通過難以忘懷的景色和真實藝術表演，我們為您提供對這個令人驚嘆的島嶼風光和無與倫比的文化更深入的理解。';
  String get gaia => '凱婭酒店萬隆';
  String get gaiaDescription => '佔地超過2.3公頃，位於面向美麗山脈的山谷中，享有令人驚嘆的城市美景，The Gaia Hotel Bandung提供的不僅僅是5星級的優雅。擁有各種設施、豐富的活動和活動，以及藝術裝置，您在住宿期間總是可以看到、做、享受的事情。';
  String get kempin => '雅加達肯賓斯基酒店';
  String get kempinDescription => '在雅加達的我們的5星級酒店中體驗奢華。酒店靠近Grand Indonesia購物中心，提供理想的位置，靠近辦公樓、旅遊勝地、大使館和Sudirman Central Boulevard區。您可以盡情享受卓越的服務、多樣的設施以及六家令人印象深刻的餐廳。一個令人難忘的入住體驗等待著雅加達充滿活力的遊客。';
  String get bandar => '雅加達Bandar Djakarta';
  String get bandarDescription => '作為一個大都市，很多人忽略了雅加達仍然是擁有最新鮮海鮮的海濱城市。雅加達Bandar Djakarta以現代氛圍呈獻了一些最好的海鮮美食。遊客可以從水族館中直接選擇新鮮的魚、螃蟹、蛤蜊和蝦。即使在雅加達有眾多食物和餐廳的情況下，這家餐廳仍然需求旺盛，每天為數百人提供服務，甚至在特殊活動上更多。';
  String get blue => '藍色露台';
  String get blueDesription => '在這個藍色露台上中和雅加達的一部分毒素，這裡的菜單非常適合那些注重健康的人。菜單上滿是豐富多彩的主菜和沙拉，全部擺放整齊，並用美味的花朵裝飾。他們還提供包括鳳梨、牛油果和雞蛋的健康早餐碗。';
  String get kudeta1 => 'Ku De Ta';
  String get kudeta1Description => 'Ku De Ta是巴厘島的標誌，透過食物、飲料和從黎明到日落充滿感情的配樂分享時刻。位於Seminyak的沙灘上，這個美麗的海濱空間被設計成陽光灑落的日子和難忘的島嶼之夜。';
  String get kudeta1ads => '地址：峇里岛巴東县库塔区Seminyak，Kayu Aya路9号';
  String get kudeta1open => '营业时间：\n周日至周四：上午08:00 - 凌晨12:00\n周五至周六：上午08:00 - 凌晨01:00';
  String get kudetaContact => '联系方式：+62 361 736969';
  String get usa => '美國';
  String get male => '男';
  String get n1=> '喜歡你的貼文';
  String get n2=> '評論了你的照片';
  String get n3=> '開始關注你';
  String get n4=> '喜歡你的貼文';
  String get a1=>'自然景觀';
  String get a2=>'購物區';
  String get a3=>'歷史古蹟';
  String get a4=>'宗教場所';
  String get a5=>'主題公園';
  String get b1=>'蘇門答臘';
  String get b2=>'爪哇';
  String get b3=>'婆羅洲';
  String get b4=>'蘇拉威西';
  String get b5=>'巴布亞';
  String get c1=>'北部';
  String get c2=>'中部';
  String get c3=>'南部';
  String get c4=>'東部';
  String get d1=>'北部';
  String get d2=>'南部';
  String get d3=>'西部';
  String get d4=>'東部';

  String get z1=>'行程';
  String get z2=>'選擇國家';
  String get z3=>'選擇地區';
  String get z4=>'選擇類別';
  String get z5=>'從';
  String get z6=>'到';
  String get z7=>'出發預估';
  String get z8=>'到達預估';
  String get z9=>'生成的行程';
  String get z10=>'評價這個行程：';
  String get z11 => '選擇日期';
  String get z12 => '選擇時間';
  String get z13=>'包括到達和出發時間、活動、餐飲和著名景點等詳細資訊。 確保日程安排良好並遵循邏輯順序。 此外，還提供有關具體遊覽地點、餐飲選擇和風景優美的散步的建議。 使用對話式和資訊豐富的語氣使生成的行程方便用戶使用。 我需要列出我的行程的每個時間段。 例如： 9:00: 去哪裡 10:00: 去哪裡';
  String get z14 => '建立時間';
  String get z15 => '到達';
  String get z16 => '去旅行';
  String get z17 => '我喜歡';

}

